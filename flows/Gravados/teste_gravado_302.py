# teste_gravado_302.py
# Gerado pelo Recorder (DSL fc) em 2026-06-14 14:57
...
if pytest is not None:
    ...
    def _fluxo():
        setup_logging(log_name="teste_gravado_302_test", json_output=True)
        executar()
        yield
        fc.reset()

    @pytest.mark.e2e
    def test_teste_gravado_302():
        # TODO: asserts de validação no banco aqui (fc.db.assert_saved / comparar).
        pass


def run() -> int:
    setup_logging(log_name="teste_gravado_302", json_output=True)
    imprimir_inicio("Teste_Gravado 302", "Teste gerado pelo Recorder (DSL fc)")
    try:
        executar()
        ...
        screenshot_on_failure("teste_gravado_302_falha")
        return 1
    finally:
        fc.reset()
