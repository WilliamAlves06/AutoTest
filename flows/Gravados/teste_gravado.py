# teste_gravado.py
# Gerado pelo Recorder (DSL fc) em 2026-06-29 23:27
...
def executar() -> None:
    ...
    fc.field("CUNSULTA").type("1").press("{ENTER}")
    fc.field("Filial").should_have_text("MATRIZ")
...
if pytest is not None:
    ...
    def _fluxo():
        setup_logging(log_name="teste_gravado_test", json_output=True)
        executar()
        yield
        fc.reset()

    @pytest.mark.e2e
    def test_teste_gravado():
...
def run() -> int:
    ...
    imprimir_inicio("Teste_Gravado", "Teste gerado pelo Recorder (DSL fc)")
    try:
        executar()
        ...
        screenshot_on_failure("teste_gravado_falha")
        return 1
    finally:
        fc.reset()
